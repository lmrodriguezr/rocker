...
class ROCker
   ...
   @@DEFAULTS.merge!({:positive=>[], :negative=>[], :seqdepth=>0.03, :readlen=>100, :minovl=>50,
      # Ext. Software
   ...
   def build!
	 ...
	 puts "  * downloading #{protein_set[:+].size} sequence(s) in positive set." unless @o[:q]
	 $stderr.puts "   # #{protein_set[:+].ids}" if @o[:debug]
	 protein_set[:+].download(ref_file)
      end
      [:+, :-].each do |set|
         unless protein_set[set].empty?
	    puts "  * linking genomes from #{protein_set[set].size} [#{set.to_s}] sequence(s)." unless @o[:q]
	 ...
	 puts "  * downloading and parsing #{genome_set[:+].size} GFF3 document(s) in #{thrs} threads." unless @o[:q]
	 $stderr.puts "   # Looking for translations: #{protein_set[:+].tranids}" if @o[:debug]
	 ...
	 (0 .. (thrs-1)).each do |thr_i|
	    ...
	    thr_obj << json_file
	    fork do
		  get_coords_from_gff3(ids_to_parse, protein_set[:+], thr_i, json_file)
	    end
	 end
	 # Combine results
	 Process.waitall
	 ...
	 ofh.print JSON.pretty_generate({:positive_coords=>positive_coords, :taxa_pos=>genome_set[:+].taxa, :taxa_neg=>genome_set[:-].taxa})
	 ofh.close
      end # if @o[:reuse] and File.size? coords_file ... else
      unless @o[:pertaxon].nil?
	 puts "  Using #{[:+,:-].map{|set| genome_set[set].size }.reduce(:+)} genome(s) after filtering by #{@o[:pertaxon]}." unless @o[:q]
      end
	 ...
	 puts "  * downloading #{genome_set[:all].size} genome(s) in FastA." unless @o[:q]
